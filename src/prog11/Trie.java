...
public class Trie<V> extends AbstractMap<String, V> {
	...
	private Node<V> find(String key, int iKey, List<Node<V>> list) {
		...
		if (iSub == node.sub.length() && iKey != key.length()) {
	...
	private V put(String key, int iKey, V value, List<Node<V>> list) {
		...
		// node to list.
		if (iNode == list.size() || keyChar < list.get(iNode).sub.charAt(0)) {
			String newSub = key.substring(iKey, key.length());
			// Node<Integer> newNode = new Node<Integer>(newSub,key, (int)value);
			Node<V> newNode = new Node<V>(newSub, key, (V) value);
			list.add(iNode, (Node<V>) newNode);
			size++;
			return null;
		}
		...
		if (iSub != node.sub.length() && iKey == key.length()) {
			 ...
			 Node<V> newNode = new Node<V>(node.sub.substring(iSub),node.key,node.value);
			 //node.list = newNode.list;
			...
			//return put(key, iKey, value, node.list);
		if(iSub != node.sub.length() && iKey != key.length()) {
			String firstSub = node.sub.substring(0,iSub);
			String fullSub = node.sub.substring(iSub);
			
			Node newNode = new Node<V>(fullSub,node.key,(V)node.value);
			node.key = null;
			node.sub = firstSub;
			node.value = null;
			node.list.add(newNode);
	...
	private class NodeIterator implements Iterator<Map.Entry<String, V>> {
		...
		public boolean hasNext() {
			...
			while(!stack.empty() && stack.peek().hasNext() == false )
				stack.pop();
